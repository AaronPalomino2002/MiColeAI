    ...
    return (
        <div className="p-8 max-w-7xl mx-auto">
            ...
            {loading ? (
                ...
                <p className="text-slate-400 text-center py-16">No hay exámenes disponibles todavía.</p>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {exams.map((exam) => (
                        <div key={exam.id} className="group relative bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 transition-all hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 overflow-hidden">
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-slate-500 dark:text-slate-400 text-sm line-clamp-2 mb-6 min-h-[40px]">{exam.description}</p>
                                <div className="flex items-center justify-end pt-6 border-t border-slate-100 dark:border-slate-800">
                                    <Link href={`/exams/${exam.id}`} className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl text-xs font-black hover:scale-105 active:scale-95 transition-all">
                                        {t("startExam")}
                                    </Link>
                                </div>
                            </div>
